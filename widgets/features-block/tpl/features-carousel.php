...
<?php if ( $features_items_array ) : ?>
	<div id="<?php echo esc_attr( $holder_id ) ?>" class="tm-sc-features tm-features-carousel tm-swiper-container" <?php unique_addons_print_pre_escaped_html_attrs( $swiper_slide_data_info ); ?>>
			...
			<div class="swiper-wrapper">
				...
				<?php foreach (  $features_items_array as $features_item ) { ?>
					...
					<div class="swiper-slide elementor-repeater-item-<?php echo esc_attr( $features_item['_id'] ); ?> <?php echo esc_attr($animation);?>" data-wow-delay="<?php echo esc_attr($animation_delay);?>">
						<?php unique_addons_get_shortcode_template_part( 'features-item', $_skin, 'features-block/tpl', $settings, false ); ?>
					</div>
				<?php } ?>
				<!-- end of the loop -->
			</div>
