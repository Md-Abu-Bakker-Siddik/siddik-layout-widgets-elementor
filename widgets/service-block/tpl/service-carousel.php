...
<?php if ( $service_items_array ) : ?>
	<div id="<?php echo esc_attr( $holder_id ) ?>" class="tm-sc-service tm-service-carousel tm-swiper-container <?php if( !empty($classes) ) echo esc_attr(implode(' ', $classes)); ?>" <?php unique_addons_print_pre_escaped_html_attrs( $swiper_slide_data_info ); ?>>
			...
			<div class="swiper-wrapper">
				...
				<?php foreach (  $service_items_array as $service_item ) { ?>
					...
					<div class="swiper-slide elementor-repeater-item-<?php echo esc_attr( $service_item['_id'] ); ?> <?php echo esc_attr($animation);?>" data-wow-delay="<?php echo esc_attr($animation_delay);?>">
						<?php unique_addons_get_shortcode_template_part( 'service-item', $_skin, 'service-block/tpl', $settings, false ); ?>
					</div>
				<?php } ?>
				<!-- end of the loop -->
			</div>
